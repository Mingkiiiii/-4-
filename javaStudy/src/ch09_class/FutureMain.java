...
public class FutureMain {
	...
	public static void main(String[] args) {
		...
		stuList.add(new FutureStudent("김기찬", "kim gi chanchan"));
		stuList.add(new FutureStudent("김대영", "kim dae young"));
		stuList.add(new FutureStudent("김동우", "kim dongwoo"));
		stuList.add(new FutureStudent("김동현", "kimdonghyun"));
		stuList.add(new FutureStudent("김명기", "kim myeonggi"));
		stuList.add(new FutureStudent("김영주", "kim youngju"));
		stuList.add(new FutureStudent("김유정", "kimyujeong"));
		...
		stuList.add(new FutureStudent("오정연", "Oh jeong yeon"));
		stuList.add(new FutureStudent("유하영", "yoo hayoung"));
		stuList.add(new FutureStudent("이예진", "lee yejin"));
		stuList.add(new FutureStudent("이용빈", "leeyongbin"));
...
//		Collections.sort(stuList, (stuA, stuB) -> stuB.getLevel() - stuA.getLevel());
		...
		Collections.sort(stuList, (stuA, stuB) -> {
			...
			//level이 같다면, exp에 대해 비교
			return stuB.getExp() - stuA.getExp();
		});
		// Collection.sort 메서드는 두 가지 파라미터를 받음
		// 여기에서 (stuA, stuB) 두 학생 객체를 비교
		// stuB.getLevel() - stuA.getLevel() stuB학생과 stuA학생의 값이 양수이면 stuB가 stuA보다 높은 레벨로 간주됨.
		// 리스트에서 stuB가 stuA보다 앞에 위치됨
